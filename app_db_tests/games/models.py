...
class Score(models.Model):
    player = models.ForeignKey(
        User, related_name="player_score", on_delete=models.CASCADE
    )
...
class PlayerGameInfo(models.Model):
    player = models.ForeignKey(
        User, related_name="games_info", on_delete=models.CASCADE
    )
...
class Game(models.Model):
    owner = models.ForeignKey(
        User, related_name="games_owner", on_delete=models.CASCADE
    )
    title = models.CharField(max_length=200)
    created = models.DateTimeField(auto_now_add=True)
    players = models.ManyToManyField(
        PlayerGameInfo, related_name="game_players_info", blank=True
    )
...
class Round(models.Model):
    ...
    players_scores = models.ManyToManyField(
...
class ItemBase(models.Model):
    ...
    def render(self):
        return render_to_string(
            f"games/content/{self._meta.model_name}.html", {"item": self}
        )
